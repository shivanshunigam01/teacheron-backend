...
export async function sendRequirementSubmittedEmail({ studentEmail, studentName, requirementTitle }) {
  ...
  const { subject, html, text } = buildRequirementSubmittedEmail({
    studentName,
    requirementTitle,
    myPostsUrl: `${clientUrl}/my-posts`,
  });

  try {
    const result = await sendMail({ to: studentEmail, subject, html, text });
    if (result.stub) {
      logger.warn('[requirement-email] SMTP not configured (submitted)', { to: studentEmail });
      return { sent: false, stub: true, reason: result.reason || 'not_configured' };
    }
    logger.info(`[requirement-email] submitted notice sent to ${studentEmail}`);
    return { sent: true, stub: false };
  } catch (err) {
    logger.error(`[requirement-email] submitted failed for ${studentEmail}: ${err.message}`);
